...
class TransitionManager: NSObject, UIViewControllerAnimatedTransitioning, UIViewControllerTransitioningDelegate {
    ...
    func animateTransition(transitionContext: UIViewControllerContextTransitioning) {
        ...
        if let introVC = fromViewController as? IntroViewController, navVC = toViewController as? UINavigationController {
            let _ = navVC.viewControllers[0] as! PhotoCaptureViewController
            container.insertSubview(toView, belowSubview: fromView)
            if introVC.presentedCameraPermissionRequest {
                UIView.animateWithDuration(duration / 3, delay: 0, usingSpringWithDamping: 1, initialSpringVelocity: 0, options: [], animations: { () -> Void in
                ...
                UIView.animateWithDuration(duration / 3, delay: 0, usingSpringWithDamping: 1, initialSpringVelocity: 0, options: [], animations: { () -> Void in
                    ...
                    UIView.animateWithDuration(duration / 3, delay: 0, usingSpringWithDamping: 0.4, initialSpringVelocity: 0, options: [], animations: { () -> Void in
